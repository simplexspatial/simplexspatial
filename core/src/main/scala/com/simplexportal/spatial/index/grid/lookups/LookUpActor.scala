...
object LookUpActor {
  ...
  final case class GetResponse[K,V](key: K, maybeValue: Option[V]) extends Response
  final case class GetsResponse[K,V](gets: Seq[GetResponse[K,V]]) extends Response

  trait Command extends LookUpMessage
  final case class Put[K,V](key: K, value: V, replyTo: Option[ActorRef[Done]]) extends Command
  final case class Get[K,V](key: K, replyTo: ActorRef[GetResponse[K,V]]) extends Command
  final case class Gets[K,V](keys: Seq[K], replyTo: ActorRef[GetsResponse[K,V]]) extends Command
  ...
  private def onCommand[K,V](
    ...
    command match {
      ...
      case cmd: Gets[K @unchecked, V @unchecked] =>
        cmd.replyTo ! GetsResponse(cmd.keys.map(key => GetResponse(key, table.get(key))))
        Effect.none

...
trait TileIndexActorDataset {
  ...
  val exampleTileCommands: Seq[protocol.BatchActions] = Seq(
    protocol.AddNode(1, 7, 3, Map("nodeAttrKey" -> "nodeAttrValue")),
    protocol.AddNode(2, 7, 10, Map.empty),
    protocol.AddNode(3, 3, 10, Map.empty),
    protocol.AddNode(4, 3, 16, Map.empty),
    protocol.AddNode(5, 4, 5, Map.empty),
    protocol.AddNode(6, 2, 5, Map.empty),
    protocol.AddWay(100, Seq(5, 6, 3), Map("wayAttrKey" -> "wayAttrValue")),
    protocol.AddWay(101, Seq(1, 2, 3, 4), Map.empty)
  )
}
